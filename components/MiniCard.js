export default function MiniCard({loading, title, data, indicator, label, isSameLabel}){
    const styleLabel = !isSameLabel ? 'bg-amber-500' : label === -1 ? 'bg-red-500' : 'bg-green-500';
    ...
    return (
        <div className="flex justify-center items-center gap-y-2 flex-col shadow-sm rounded-xl h-28 sm:h-32 w-32 xs:w-36 sm:w-48 p-2 sm:p-4 bg-white relative">
            <span className={`${!label ? 'hidden' : ''} ${label ? styleLabel : ''} absolute top-0 left-0 text-[.6rem] py-0.5 px-1 rounded-sm text-white`}>{label ===  1 ? 'Beli/Jual' : 'Tahan'}</span>
            {loading ?
